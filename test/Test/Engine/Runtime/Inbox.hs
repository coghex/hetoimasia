...
-- collecting sink, with synthetic typed failures. Each component context is a
-- traced resource whose release records whether the inbox still admits a
-- message, through the send endpoint the start returned, so the trace shows the
-- inbox closed before the component was torn down.
--
