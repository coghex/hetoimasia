...
-- 'awaitBlockedOnSTM' says it has parked in a transaction, producers start from
-- a gate, and worker outcomes are read raw with 'awaitTerminal' before a
-- supervised wait is entered. No example sleeps, and none asserts an order
-- between producers that only timing could decide; 'boundedSupervision' only
-- stops an example that has already hung.
--
-- The supervision examples run the real 'withSupervision' and 'awaitSupervised'
-- with the fixtures of "Test.Engine.Runtime.Supervision.Support".
module Test.Engine.Messaging.Channel (spec) where
...
spec = do
  ...
  describe "Channel admission and order" $ do
    ...
    it "keeps each of several concurrent producers' entries in order"
      (boundedSupervision testConcurrentProducers)
  ...
  describe "Channel counters and transactions" $ do
    it "conserves accepted entries and keeps high-water monotone through contention, rollback, failed admission, drain, and abort"
      (boundedSupervision testConservation)
          ...
          case failureCause evidence of
            EngineOrigin origin → do
              ...
              fmap (sourceFile . siteLocation) (originSite origin)
                `shouldSatisfy` maybe False ("Test/Engine/Messaging/Channel.hs" `Text.isSuffixOf`)
