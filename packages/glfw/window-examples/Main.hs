-- | The window model, window command, window control, window host, monitor
-- inventory, input feed, and window mode examples, run as an executable so the private window and monitor drivers and
...
main ∷ IO ()
main = hspec (describe "GLFW" (Window.spec >> Command.spec >> Control.spec >> Host.spec >> Dynamic.spec >> Monitor.spec >> Input.spec >> Mode.spec))
