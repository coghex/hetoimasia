...
-- | A group's lifecycle phase, as 'groupStatus' reports it.
data GroupPhase
  = GroupOpen
    -- ^ Registration is open.
  | GroupClosing
    -- ^ Closing has begun and the 'GroupReport' is not yet published.
  | GroupClosed
    -- ^ The 'GroupReport' is published. This says nothing about what the
    -- group's owner does next.
  deriving (Eq, Show)
...
data OutstandingWorker = OutstandingWorker
  ...
  , outstandingRequested ∷ !Requested
    -- ^ The strongest request made so far. 'CancelWasRequested' is also a
    -- stop request.
  , outstandingCancelDelivered ∷ !Bool
    -- ^ A cancellation helper's 'throwTo' returned, so 'WorkerCancelled' was
    -- raised in the worker's thread. A reserved, blocked, skipped, or failed
    -- attempt is not delivery, and delivery proves neither that worker code
    -- handled the exception nor that the worker terminated. Never cleared.
...
data GroupStatus = GroupStatus
  ...
  , statusOutstanding ∷ ![OutstandingWorker]
    -- ^ In registration order; always empty once 'GroupClosed'.
  }
  deriving (Eq, Show)
...
newtype GroupProbe = GroupProbe
  { probeHelperSettling ∷ WorkerId → IO ()
    ...
    -- 'groupStatus' and the drain cannot complete. A synchronous failure it
    -- raises is discarded.
  }
