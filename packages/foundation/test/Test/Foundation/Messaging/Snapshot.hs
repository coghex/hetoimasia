...
-- 'awaitBlockedOnSTM' says it has parked in a transaction, and concurrent
-- readers start from a gate. That a read does not wait is proven with 'orElse',
-- which takes its alternative only if the read retried. No example sleeps;
-- 'boundedExample' only stops an example that has already hung.
--
-- The examples that settle a supervised worker's outcome around a snapshot wait
-- exercise the runtime's supervision and are registered by the root suite's
-- @Runtime@ group, in "Test.Engine.Runtime.Messaging".
module Test.Foundation.Messaging.Snapshot (spec) where

import Control.Concurrent (forkIO, myThreadId)
import Control.Concurrent.MVar (newEmptyMVar, putMVar, readMVar, takeMVar)
import Control.Concurrent.STM (STM, atomically, orElse, retry, throwSTM)
...
spec = do
  ...
  describe "Snapshot composition" $ do
    it "leaves a blocked waiting read through a worker's stop request"
      (boundedExample testStopDuringWait)
          ...
          case failureCause evidence of
            EngineOrigin origin → do
              ...
              fmap (sourceFile . siteLocation) (originSite origin)
                `shouldSatisfy` maybe False ("Test/Foundation/Messaging/Snapshot.hs" `Text.isSuffixOf`)
