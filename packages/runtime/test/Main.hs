-- | Entry point for the runtime package's suite.
--
-- The examples live in the component specs 'Test.Runtime.Spec' composes; this
...
import qualified Test.Runtime.Spec as Runtime
import Test.Hspec.Runner (Config (configFailOnEmpty), defaultConfig, hspecWith)

main ∷ IO ()
main = hspecWith defaultConfig { configFailOnEmpty = True } Runtime.spec
