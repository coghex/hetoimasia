...
import Hetoimasia.Scripting.Lua.Internal.Protocol.Request
  ...
  , SettlementKind (KindCancelled, KindResult)
  )
...
spec = describe "requests" $ do
  it "settles once, and a duplicate reply neither overwrites nor disappears" $ do
    ...
    (b, identity) ← ok (acceptRequest owner (RequestName 1) firstGeneration endpoint a)
  ...
  it "lets a task wait only on a request it owns" $ do
    ...
    (c, identity) ← ok (acceptRequest first (RequestName 1) firstGeneration endpoint b)
    (_, refusal) ←
  ...
  it "counts a request against the cap until both obligations are discharged" $ do
    session ← openSession
    (a, owner) ← runningTask 1 session
    (b, first) ← ok (acceptRequest owner (RequestName 1) firstGeneration endpoint a)
    (c, _) ← ok (acceptRequest owner (RequestName 2) firstGeneration endpoint b)
    (d, refusal) ← rejected (acceptRequest owner (RequestName 3) firstGeneration endpoint c)
    ...
    (g, stillFull) ← rejected (acceptRequest owner (RequestName 3) firstGeneration endpoint f)
    ...
    (final, _) ← ok (acceptRequest owner (RequestName 3) firstGeneration endpoint h)
    Map.size (sessionRequests final) `shouldBe` 2

  it "separates the same request name's generations" $ do
    session ← openSession
    (a, owner) ← runningTask 1 session
    (b, older) ← ok (acceptRequest owner (RequestName 1) firstGeneration endpoint a)
    (c, newer) ← ok (acceptRequest owner (RequestName 1) (Generation 1) endpoint b)
  ...
  it "refuses an oversize reply payload without settling, and counts it" $ do
    session ← openSession
    (a, owner) ← runningTask 1 session
    (b, identity) ← ok (acceptRequest owner (RequestName 1) firstGeneration endpoint a)
