...
spec = describe "owner progress" $ do
  ...
  it "walks the idle backoff schedule while nothing progresses, and restarts it on demand" $ do
    ...
    nextDeadline (atMilliseconds 0) loaded `shouldBe` TurnAt (atMilliseconds 5)
    ...
    deadlines `shouldBe` map (TurnAt . atMilliseconds) [15, 35, 75, 155, 255, 355, 455]
    ...
    nextDeadline (atMilliseconds 0) steppedAgain `shouldBe` TurnAt (atMilliseconds 20)
    demanded ← admitted_ "requesting a render" (requestRender target steppedAgain)
    nextDeadline (atMilliseconds 0) demanded `shouldBe` TurnAt (atMilliseconds 0)
    let (served, _) = runProgressTurn silentEvidence (atMilliseconds 0) demanded
    nextDeadline (atMilliseconds 0) served `shouldBe` TurnAt (atMilliseconds 0)
  ...
  it "restarts the backoff on a new obligation, an observed completion and a close" $ do
    ...
    nextDeadline (atMilliseconds 0) backedOff `shouldBe` TurnAt (atMilliseconds 40)

    -- A new obligation.
    (obliged, _) ← admitted "reserving another frame" (reserveFrame target backedOff)
    nextDeadline (atMilliseconds 0) obliged `shouldBe` TurnAt (atMilliseconds 5)

    -- An observed completion.
    let backedOffAgain = walk 3 obliged
    nextDeadline (atMilliseconds 0) backedOffAgain `shouldBe` TurnAt (atMilliseconds 40)
    completed ← admitted_ "completing" (recordCompletion (atMilliseconds 1) (SubmissionCompleted submission) backedOffAgain)
    nextDeadline (atMilliseconds 0) completed `shouldBe` TurnAt (atMilliseconds 5)

    -- A close transition.
    let backedOffOnceMore = walk 3 completed
    nextDeadline (atMilliseconds 0) backedOffOnceMore `shouldBe` TurnAt (atMilliseconds 40)
    closed ← admitted_ "closing" (closeTarget target backedOffOnceMore)
    nextDeadline (atMilliseconds 0) closed `shouldBe` TurnAt (atMilliseconds 5)
  ...
  it "restarts the backoff for retirement work, whoever created it" $ do
    ...
    nextDeadline (atMilliseconds 0) retired `shouldBe` TurnAt (atMilliseconds 5)
    ...
    nextDeadline (atMilliseconds 0) ended `shouldBe` TurnAt (atMilliseconds 5)
    ...
    nextDeadline (atMilliseconds 0) released `shouldBe` TurnAt (atMilliseconds 5)
    let backedOffLast = walk 3 released
    nextDeadline (atMilliseconds 0) backedOffLast `shouldBe` TurnAt (atMilliseconds 40)
    settled ← admitted_ "ending the resource's CPU use" (endResourceCpuUse resource backedOffLast)
    nextDeadline (atMilliseconds 0) settled `shouldBe` TurnAt (atMilliseconds 5)
  ...
  it "restarts the backoff when a construction fails or is superseded" $ do
    ...
    nextDeadline (atMilliseconds 0) failed `shouldBe` TurnAt (atMilliseconds 5)
    ...
    nextDeadline (atMilliseconds 0) secondBackedOff `shouldBe` TurnAt (atMilliseconds 40)
    ...
    nextDeadline (atMilliseconds 0) superseded `shouldBe` TurnAt (atMilliseconds 5)
  ...
  it "drops a suspended target's render deadline while keeping its retirement demand" $ do
    ...
    nextDeadline (atMilliseconds 0) suspended `shouldBe` TurnAt (atMilliseconds 5)
    ...
    nextDeadline (atMilliseconds 0) resumed `shouldBe` TurnAt (atMilliseconds 0)
  ...
  it "advertises a recovery retry deadline on an otherwise idle target" $ do
    ...
    nextDeadline (atMilliseconds 0) active `shouldBe` NoTurnNeeded
    ...
    nextDeadline (atMilliseconds 0) begun `shouldBe` NoTurnNeeded
    ...
    nextDeadline (atMilliseconds 0) failed `shouldBe` TurnAt (atMilliseconds 100)
    -- It is an absolute instant, not an interval, so reading later does not move it.
    nextDeadline (atMilliseconds 50) failed `shouldBe` TurnAt (atMilliseconds 100)
    ...
    nextDeadline (atMilliseconds 100) again `shouldBe` NoTurnNeeded
  ...
  it "advertises the healthy-progress deadline that resets a recovery episode" $ do
    ...
    nextDeadline (atMilliseconds 3000) spent `shouldBe` NoTurnNeeded
    ...
    nextDeadline (atMilliseconds 3000) cycled `shouldBe` TurnAt (atMilliseconds 4000)
    ...
    nextDeadline (atMilliseconds 4000) healthy `shouldBe` NoTurnNeeded

  it "reports an unschedulable turn rather than none when the deadline arithmetic overflows" $ do
    model ← freshModelWith defaultBudgetRequest {requestedFrameSlots = 2}
    (active, target, _) ← activeTarget 2 model
    (loaded, _) ← enqueueFrames target 1 active
    -- There is a pending submission, so a turn is certainly needed; the backoff
    -- interval added to the last representable instant is what does not fit.
    pendingObligations loaded `shouldSatisfy` (> 0)
    nextDeadline lastInstant loaded `shouldBe` TurnUnschedulable
    -- The same model an instant earlier in the range schedules normally, so this
    -- is the arithmetic and not the work.
    nextDeadline (atMilliseconds 0) loaded `shouldBe` TurnAt (atMilliseconds 5)

    -- An overflowing healthy-progress deadline is reported the same way instead
    -- of vanishing from the schedule.
    spent ← spendEpisode target loaded 3 0
    cycled ← completeCycle target lastInstant spent
    nextDeadline lastInstant cycled `shouldBe` TurnUnschedulable

    -- Render demand still answers immediately: that instant needs no arithmetic.
    demanded ← admitted_ "requesting a render" (requestRender target loaded)
    nextDeadline lastInstant demanded `shouldBe` TurnAt lastInstant
  ...
  it "has no deadline at all once nothing is pending" $ do
    ...
    nextDeadline (atMilliseconds 1) disposed `shouldBe` NoTurnNeeded
