# Archived project memory before the 2026-09-17 reconciliation

This preserves the prior memory and pending documentation notes. Dates, counts,
open-issue statements and implementation claims below are historical snapshots,
including overlapping notes from master and docs-wip. They are not current
...
[the foundation design](../engine_foundation_design.md).

## Current handoff — pre-Vulkan scheduling and lifetime design

- Code baseline: `master@e2d30ea`. Four active Cabal packages: root consumer/tests,
...
- GLFW issues #87–#100 merged through PRs #101–#114. The
  [review report](../project_review_114-101.md) identified four repairs:
  ...
  under repeated cancellation. All four merged through PRs #119–#122.
- Retained implementation detail from the merged repair notes: #115 is repaired (PR #119); #116 is repaired:
...
### Pending handoff notes from the documentation branch (historical)

- The [repair review](../project_review_122-119.md) found one remaining defect:
  a borderless window's recovery association does not follow later observed
  movement between live monitors. The owner approved and filed #123. Four
  additional CPU-seam Hspec examples reproduce it; #115/#117/#118 had no further
  confirmed findings. Fix #123 before window-controller integration.
- That review passed the build, focused GLFW engine checks, and 13 scripted
  fixture examples locally without acquiring a native session. Final PR #122's
  Linux native job ran 60 examples with zero failures and one physical-hotplug
  pending example; master reused that evidence. See the report for exact runs.
- Epics #86/#49 remain open and their original checklists lag merged children;
  tracker housekeeping is separate from these document edits.
- Next work: process the scheduling and window/graphics-lifetime designs,
...
- Pre-Vulkan backlog (2026-09-16): `docs/runtime_scheduling_design.md` owns
  TIME-1–TIME-6; `docs/window_graphics_lifetime_design.md` owns LIFE-1–LIFE-4.
  Both are ready for issue processing after the final review on 2026-09-16;
  no TIME/LIFE tracker items have been filed. The owner
  accepted event/deadline/fixed-step scheduling with configurable limits,
  per-window render suspension while simulation remains application-owned,
  one exclusive graphics owner per window, and preserving admitted work while
  warning once and using bounded polling after expected native wake failure.
  The owner also accepted protected main-thread graphics retirement after
  worker drain and before dependency release, retaining resources while safety
  is unknown. Actual Vulkan completion and
  surface integration stay in `docs/vulkan_backend_design.md`. Follow-up #123
  is filed; CPU timing/model work can proceed independently of that repair.
- Test organization discussion (2026-09-16): the owner asked about package-owned,
  independently runnable suites. Inspection confirms the root engine suite
  combines foundation/runtime/GLFW/console dependencies and launches the private
  GLFW model executable. The recommendation is package-local foundation/runtime
  suites, separate GLFW model/native suites, and root console/integration tests,
  preserving external-client opacity checks and explicitly updating CI selection.
  This is a recommendation under discussion, not an approved migration or a new
  dependency of the TIME/LIFE designs.
- Owner testing policy (2026-09-16): routine local runs must not steal desktop
  focus. Before a disruptive local test session, ask for the human user's
  explicit approval, explain the disruption, and wait for acceptance before
  continuing. Necessary desktop disruption is acceptable after that approval;
  approval covers the agreed session, not unrelated later runs. An issue's
  acceptance commands alone do not grant that approval. Isolated X11 CI remains
  required when affected. Interactive local
  probes should be optional. The current catalog still has one non-optional
  native group and no optional groups; the executable/catalog split is future
  work, not an implemented gate. See AGENTS.md for the immediate agent rule.
...
- Logger accepts an injected sink and injectable clock/thread metadata, applies
  ...
  [docs/logging.md](../logging.md). Runtime invokes a supplied action and
...
- CI runs the validation pipeline and the review gate described in
  [validation.md](../validation.md). `build-test` and `review-approved` are
  ...
  CI evidence is assessed independently. See [workflow.md](../workflow.md). The
...
- On 2026-09-10 the owner requested review and readiness of both discussed
  designs. [Logging](../logging_design.md) has three slices;
  [resource ownership](../resource_ownership_design.md) has four. Both were
...
- The first rendering milestone is a window with a rendered triangle, explicitly
  ...
  still need bounded design. See [the backend design](../vulkan_backend_design.md).
...
- Separate GLFW and Vulkan components and a first main-thread window/render
  ...
  `Hetoimasia.Foundation.Worker` ([docs/workers.md](../workers.md)): a group
...
- Runtime epic #52's eight children #53–#60 are merged, along with supervision
  ...
  handoff above.
- Messaging design started in [docs/messaging_design.md](../messaging_design.md)
