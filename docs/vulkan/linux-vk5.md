# The VK-5 surface bridge record, Linux
...
loader-aware GLFW surface bridge" below. It ran through the `vulkan-proof` route of `.github/workflows/ci-image.yml` (run 35999547022), inside the published image the committed `tools/ci-image/descriptor.json` names, on the isolated X11 display `tools/display/x11.sh` starts and supplies its own consent for. No desktop session was entered.
...
- the session copied the platform's required instance extensions
  (`VK_KHR_surface`, `VK_KHR_xcb_surface`), and the instance was created with them;
...
- after the session terminated the shim held null and the capability was
  restored;
- a second capability's Wayland session, pointed at a display no compositor
  serves, failed `glfwInit` with GLFW's own `Wayland: Failed to connect to
  display`, and afterwards the shim again held null and that capability was
  restored.
...
- the binding's vkGetInstanceProcAddr: 0x00007fecfa4333d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
- GLFW's vkGetInstanceProcAddr: 0x00007fecfa4333d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
- the binding's vkCreateDevice: 0x00007fecfa433090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
- GLFW's vkCreateDevice: 0x00007fecfa433090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
- a device-level entry point: 0x00007fecd864b850 in /usr/lib/x86_64-linux-gnu/libVkLayer_khronos_validation.so
...
- device: llvmpipe (LLVM 20.1.2, 256 bits)
- messenger callback: 0x0000000000882690 in /candidate/dist-vulkan-proof/build/x86_64-linux/ghc-9.14.1/hetoimasia-vulkan-proof-0.1.0.0/t/vulkan-proof/opt/build/vulkan-proof/vulkan-proof
...
| vkCreateInstance | info | WARNING-CreateInstance-status-message | Validation Information: [ WARNING-CreateInstance-status-message ] Object 0: handle = 0x29086a10, type = VK_OBJECT_TYPE_INSTANCE; \| MessageID = 0x23dfd876 \| v... |
...
- binding vkGetInstanceProcAddr: 0x00007fecfa4333d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
- capability made from: 0x00007fecfa4333d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
- shim setting while the session was live: 0x00007fecfa4333d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
- GLFW resolved vkGetInstanceProcAddr to: 0x00007fecfa4333d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
- capability while the session was live: IntegrationInstalled
- binding vkCreateDevice: 0x00007fecfa433090 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkCreateDevice
- GLFW vkCreateDevice: 0x00007fecfa433090 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkCreateDevice
- required instance extensions, as copied: VK_KHR_surface, VK_KHR_xcb_surface
- surface creation: SurfaceCreated (WindowSurface (SurfaceObligation (AttachmentId (WindowId 1) 1) 731325632))
- surface handle: 731325632
...
VK_LAYER_PATH = /opt/hetoimasia/native/glfw/vulkan/share/vulkan/explicit_layer.d
## The shared loader
the binding dispatches through 0x00007fecfa4333d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
the binding's loader is the recorded loader /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275
GLFW resolves the same name to 0x00007fecfa4333d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
GLFW requires VK_KHR_surface, VK_KHR_xcb_surface
## The instance
the loader reports instance version 1.3.275
the binding resolves vkCreateDevice to 0x00007fecfa433090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
GLFW resolves vkCreateDevice to 0x00007fecfa433090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
...
the binding dispatches image release through 0x00007fecd86952e0 in /usr/lib/x86_64-linux-gnu/libVkLayer_khronos_validation.so
...
the capability was made from 0x00007fecfa4333d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
while the session is live the shim holds 0x00007fecfa4333d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
GLFW resolves vkGetInstanceProcAddr to 0x00007fecfa4333d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
the session copied the required extensions VK_KHR_surface, VK_KHR_xcb_surface
the binding resolves vkCreateDevice to 0x00007fecfa433090 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkCreateDevice
GLFW resolves vkCreateDevice to 0x00007fecfa433090 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkCreateDevice
created surface 731325632 for the attached window
