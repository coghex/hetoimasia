# The VK-7 Vulkan roots record, Linux
...
> the one above, because the VK-7 section is what this record is retained for.
...
`vulkan-proof` route of `.github/workflows/ci-image.yml` (run 36053829269),
...
It was produced from repository revision `326e4e9ce96befdaca29ce0070f80bf4a2cf3c5d`
and source digest
`b22f7efb6812c62cb4cac36f1599ea4f354f64f447ecfb81aef84aebb7589567`; the digest
...
What the VK-7 section shows, on this platform:

- the integration package's own composition ran the native package's
  production roots under the GLFW package's supervised graphics owner: the
  instance, created with the validation layer and the capture's messenger
  chained into its create info, and the explicit messenger were created, and the
  instance leased to the surface bridge;
- both windows' surfaces were created through GLFW on the process main thread;
  the device (`llvmpipe`) was selected against the first and created once,
  queue family 0, and the second surface was checked against that family; both
  targets were admitted as required;
- after the first-created window was closed, its surface had been destroyed and
  its window released, while the device, the instance and the second target
  stayed live;
- at the exit the second surface, the device, the explicit messenger and the
  instance were destroyed in that order; every root's creation and destruction
  and every surface's destruction ran on one Haskell thread, the owner's, off
  the main OS thread, each OS thread read through `pthread_self` at the call;
- the explicit messenger received 0 reports during child teardown and the
  create-info messenger 1 during `vkDestroyInstance`; the capture admitted and
  delivered all 77 reports it was offered, dropped and truncated none, latched no
  error, and was quiesced by the instance's destruction.
...
- source digest: b22f7efb6812c62cb4cac36f1599ea4f354f64f447ecfb81aef84aebb7589567
- repository revision: 326e4e9ce96befdaca29ce0070f80bf4a2cf3c5d
...
- the binding's vkGetInstanceProcAddr: 0x00007fd3ef0723d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
- GLFW's vkGetInstanceProcAddr: 0x00007fd3ef0723d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
- the binding's vkCreateDevice: 0x00007fd3ef072090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
- GLFW's vkCreateDevice: 0x00007fd3ef072090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
- a device-level entry point: 0x00007fd3ca3ab850 in /usr/lib/x86_64-linux-gnu/libVkLayer_khronos_validation.so
...
- device: llvmpipe (LLVM 20.1.2, 256 bits)
- messenger callback: 0x00000000006fe780 in /candidate/dist-vulkan-proof/build/x86_64-linux/ghc-9.14.1/hetoimasia-vulkan-proof-0.1.0.0/t/vulkan-proof/opt/build/vulkan-proof/vulkan-proof
...
| vkCreateInstance | info | WARNING-CreateInstance-status-message | Validation Information: [ WARNING-CreateInstance-status-message ] Object 0: handle = 0xe914fa0, type = VK_OBJECT_TYPE_INSTANCE; \| MessageID = 0x23dfd876 \| vk... |
...
- binding vkGetInstanceProcAddr: 0x00007fd3ef0723d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
- capability made from: 0x00007fd3ef0723d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
- shim setting while the session was live: 0x00007fd3ef0723d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
- GLFW resolved vkGetInstanceProcAddr to: 0x00007fd3ef0723d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
- capability while the session was live: IntegrationInstalled
- binding vkCreateDevice: 0x00007fd3ef072090 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkCreateDevice
- GLFW vkCreateDevice: 0x00007fd3ef072090 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkCreateDevice
- required instance extensions, as copied: VK_KHR_surface, VK_KHR_xcb_surface
- surface creation: SurfaceCreated (WindowSurface (SurfaceObligation (AttachmentId (WindowId 1) 1) 223415744))
- surface handle: 223415744
...
- failed initialization: a wayland session whose glfwInit failed: NativeFailure {nativeOutcome = NativeCallFailed, nativeReports = Reports {reportedErrors = [NativeError {nativeErrorCode = 65544, nativeErrorDescription = "Wayland: Failed to connect to display", nativeErrorTruncated = False, nativeErrorThread = ProcessMainThread}], reportsLost = 0, callbackFaults = 0}}
- shim setting after the failed initialization: 0x0000000000000000
- capability after the failed initialization: IntegrationRestored
...
- capture limits: 1024 queued records, 16384 bytes of text per record (the default is 4096; see the proof README), 16 objects per record
- main thread: OS thread 0x7fd3eeda9080, ThreadId 4
- readiness: RootsReady
- device: llvmpipe (LLVM 20.1.2, 256 bits)
- queue family: 0
- targets once both windows were handed over: RequiredTarget on surface 0x20000000002, RequiredTarget on surface 0x30000000003
...
- reports during child teardown (explicit messenger): 0
- reports during vkDestroyInstance (create-info messenger): 1
...
- capture failures: 0
- error reports: 0
- delivered to the logger: 77
- undelivered: 0
- drain worker: completed

| Native call | OS thread | Main thread | Haskell thread | Reports during it | Raised |
| --- | --- | --- | --- | --- | --- |
| vkEnumerateInstanceExtensionProperties | 0x7fd3e6ffe6c0 | no | ThreadId 89 | 0 | no |
| vkCreateInstance | 0x7fd3e6ffe6c0 | no | ThreadId 89 | 51 | no |
| vkCreateDebugUtilsMessengerEXT | 0x7fd3e6ffe6c0 | no | ThreadId 89 | 0 | no |
| glfwCreateWindowSurface | 0x7fd3eeda9080 | yes | ThreadId 4 | 0 | no |
| vkEnumeratePhysicalDevices | 0x7fd3e6ffe6c0 | no | ThreadId 89 | 12 | no |
| vkCreateDevice | 0x7fd3e6ffe6c0 | no | ThreadId 89 | 13 | no |
| glfwCreateWindowSurface | 0x7fd3eeda9080 | yes | ThreadId 4 | 0 | no |
| vkGetPhysicalDeviceSurfaceSupportKHR | 0x7fd3e6ffe6c0 | no | ThreadId 89 | 0 | no |
| vkDestroySurfaceKHR | 0x7fd3e6ffe6c0 | no | ThreadId 89 | 0 | no |
| vkDestroySurfaceKHR | 0x7fd3e6ffe6c0 | no | ThreadId 89 | 0 | no |
| vkDestroyDevice | 0x7fd3e6ffe6c0 | no | ThreadId 89 | 0 | no |
| vkDestroyDebugUtilsMessengerEXT | 0x7fd3e6ffe6c0 | no | ThreadId 89 | 0 | no |
| vkDestroyInstance | 0x7fd3e6ffe6c0 | no | ThreadId 89 | 1 | no |
...
## The environment
implicit-layer policy: VK_LOADER_LAYERS_DISABLE=~implicit~, so no implicit layer joins the chain and the explicit layers below are all of it
proving repository revision 326e4e9ce96befdaca29ce0070f80bf4a2cf3c5d
proving source digest b22f7efb6812c62cb4cac36f1599ea4f354f64f447ecfb81aef84aebb7589567
VK_DRIVER_FILES = /opt/hetoimasia/native/glfw/vulkan/share/vulkan/icd.d/lvp_icd.json
VK_LAYER_PATH = /opt/hetoimasia/native/glfw/vulkan/share/vulkan/explicit_layer.d
## The shared loader
the binding dispatches through 0x00007fd3ef0723d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
the binding's loader is the recorded loader /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275
GLFW resolves the same name to 0x00007fd3ef0723d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
GLFW requires VK_KHR_surface, VK_KHR_xcb_surface
## The instance
the loader reports instance version 1.3.275
the binding resolves vkCreateDevice to 0x00007fd3ef072090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
GLFW resolves vkCreateDevice to 0x00007fd3ef072090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
...
the binding dispatches image release through 0x00007fd3ca3f52e0 in /usr/lib/x86_64-linux-gnu/libVkLayer_khronos_validation.so
...
the capability was made from 0x00007fd3ef0723d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
while the session is live the shim holds 0x00007fd3ef0723d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
GLFW resolves vkGetInstanceProcAddr to 0x00007fd3ef0723d0 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkGetInstanceProcAddr
the session copied the required extensions VK_KHR_surface, VK_KHR_xcb_surface
the binding resolves vkCreateDevice to 0x00007fd3ef072090 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkCreateDevice
GLFW resolves vkCreateDevice to 0x00007fd3ef072090 in /usr/lib/x86_64-linux-gnu/libvulkan.so.1.3.275 as vkCreateDevice
created surface 223415744 for the attached window
...
the Wayland initialization failed: NativeFailure {nativeOutcome = NativeCallFailed, nativeReports = Reports {reportedErrors = [NativeError {nativeErrorCode = 65544, nativeErrorDescription = "Wayland: Failed to connect to display", nativeErrorTruncated = False, nativeErrorThread = ProcessMainThread}], reportsLost = 0, callbackFaults = 0}}
after it the shim holds 0x0000000000000000 and the capability is IntegrationRestored
## VK-7: the Vulkan roots under the graphics owner
the owner created the instance and its explicit messenger, and leased it to the surface bridge
both targets admitted on llvmpipe (LLVM 20.1.2, 256 bits), queue family 0
after the first window closed: device RootLive, instance RootLive, 1 target
the explicit messenger received 0 reports during child teardown
the create-info messenger received 1 reports during vkDestroyInstance
```
