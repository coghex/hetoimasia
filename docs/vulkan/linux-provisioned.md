...
This record and [macos-provisioned.md](macos-provisioned.md) carry the *same*
source digest, `85314516…`, computed independently — on macOS from a Git
checkout, and inside the container from the candidate mounted into it. As with
the VK-2 pair, that is direct evidence the two platforms proved one tree rather
than two believed to match.
...
- repository revision: 66da7d2934df2895feb7693ba5867e74eda110b7
...
- the binding's vkGetInstanceProcAddr: 0x00007efeee93d3d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
- GLFW's vkGetInstanceProcAddr: 0x00007efeee93d3d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
- the binding's vkCreateDevice: 0x00007efeee93d090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
- GLFW's vkCreateDevice: 0x00007efeee93d090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
- a device-level entry point: 0x00007efeca3ab850 in /usr/lib/x86_64-linux-gnu/libVkLayer_khronos_validation.so
...
## The environment
implicit-layer policy: VK_LOADER_LAYERS_DISABLE=~implicit~, so no implicit layer joins the chain and the explicit layers below are all of it
proving repository revision 66da7d2934df2895feb7693ba5867e74eda110b7
proving source digest 85314516b1d367ba6b7467dee4ca5421b5e7be62e3a93a71563f66c5802cb1fd
VK_DRIVER_FILES = /opt/hetoimasia/native/glfw/vulkan/share/vulkan/icd.d/lvp_icd.json
VK_LAYER_PATH = /opt/hetoimasia/native/glfw/vulkan/share/vulkan/explicit_layer.d
## The shared loader
the binding dispatches through 0x00007efeee93d3d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
GLFW resolves the same name to 0x00007efeee93d3d0 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkGetInstanceProcAddr
GLFW requires VK_KHR_surface, VK_KHR_xcb_surface
## The instance
the loader reports instance version 1.3.275
the binding resolves vkCreateDevice to 0x00007efeee93d090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
GLFW resolves vkCreateDevice to 0x00007efeee93d090 in /lib/x86_64-linux-gnu/libvulkan.so.1 as vkCreateDevice
...
the binding dispatches image release through 0x00007efeca3f52e0 in /usr/lib/x86_64-linux-gnu/libVkLayer_khronos_validation.so
