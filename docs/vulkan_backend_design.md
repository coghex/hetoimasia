...
| Provisioning | **Delivered by VK-4 (#208)**: `tools/native/vulkan.pin` names the loader, driver, validation layer, and glslang compiler on each platform, and `tools/native/vulkan.py` provisions them into a project-managed prefix at `<native prefix>/vulkan` — Linux at exactly the Ubuntu 24.04 package revisions the pin names, macOS by qualifying and adopting the machine's LunarG SDK and Homebrew MoltenVK artifacts by digest. The native manifest records every loader, driver, layer, and compiler identity, and those identities enter the recipe fingerprint, the image labels and descriptor, and the plan's toolchain map, so a worker whose inputs differ is refused and a receipt gathered under one set is not reusable under another. The throwaway container and the project-local `environment.pin` are retired; `run-proof.sh` selects its inputs from the prefix and the `vulkan-proof` route runs in the published image. macOS native qualification of the delivered prefix remains **pending consent** — see the VK-4 slice. | System packages for local exploration, but still pin the CI environment and reject incompatible capabilities explicitly. |
