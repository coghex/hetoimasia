...
* A tracked `AGENTS.md` symlink to `CLAUDE.md` is canonicalized and verified.
  Hetoimasia instead tracks `AGENTS.md` as a regular authoritative document;
  that file lands under its own name. An existing alias cannot be replaced
  locally to bypass its integrity checks.
...
def uses_agents_alias(worktree: Path) -> bool:
    """Recognize an alias on disk, in the index, or in either committed tip.

    Checking the tips preserves the upstream integrity rule even when a
    replacement regular file has already been staged. A repository that has
    always used an ordinary AGENTS.md needs no alias canonicalization.
    """
    if (worktree / ROOT_CONTRACT_ALIAS).is_symlink():
        return True
    indexed = run_git(worktree, "ls-files", "-s", "--", ROOT_CONTRACT_ALIAS).stdout
    if any(line.startswith("120000 ") for line in indexed.splitlines()):
        return True
    for ref in ("HEAD", CONTRACT_REF):
        entry = run_git(worktree, "ls-tree", ref, "--", ROOT_CONTRACT_ALIAS).stdout
...
def assess(
    ...
    try:
        ...
        if path == ROOT_CONTRACT_ALIAS and uses_agents_alias(worktree):
